...
@Autonomous(name="Vuforia Track Wheels", group = "Uselessbot")
//@Disabled
public class UselessAutoVuforiaTrack extends LinearOpMode {

    static final double     FULLSPEED             = 0.3;
    static final double     HALFSPEED             = 0.2;

    HardwareUselessbot robot   = new HardwareUselessbot(true);   // robot goes backward
...
//                if (degreesToTurn < 0 || degreesToTurn > 180) { // in landscape, negative angle is off the screen
//                    leftSpeed = 0.0;
//                    rightSpeed = 0.0;
//                } else if (degreesToTurn > 10 && degreesToTurn <160) { // point straight
//                    leftSpeed = 0.0;
//                    rightSpeed = 0.0;
//                } else if (degreesToTurn > 160) { // rotate right
//                    leftSpeed = HALFSPEED;
//                    rightSpeed = -HALFSPEED;
//                } else if (degreesToTurn < 10) { // veer left
//                    leftSpeed = -HALFSPEED;
//                    rightSpeed = HALFSPEED;
//                }

                if (Math.abs(degreesToTurn) < 100) {  // close to center, no movement
                    leftSpeed = 0.0;
                    rightSpeed = 0.0;
                } else if (degreesToTurn < 0) { // rotate right
                    leftSpeed = HALFSPEED;
                    rightSpeed = -HALFSPEED;
